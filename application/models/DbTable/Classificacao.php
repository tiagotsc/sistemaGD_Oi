...
class Application_Model_DbTable_Classificacao extends Zend_Db_Table_Abstract {
	
	/*protected $this->_db 			= Zend_Registry::get("db1");
	protected $this->_schema		= 'FSW_OSS_PADRAO';
	protected $_name = 'DOCUMENTO_TIPO';
	protected $this->_schema_name	= $this->_schema.'.'.$this->_name;*/
	
	public function init() {
		$this->_db 			= Zend_Registry::get("db1");
		$this->_schema		= 'APP_SGD';
		$this->_name		= 'CLASSIFICACAO';
		$this->_schema_name	= $this->_schema.'.'.$this->_name;
		$this->_db->getProfiler()->setEnabled(true);
		#$teste = $this->_db->fetchAll("SELECT * FROM APP_SGD.DOCUMENTO_TIPO");
		#echo '<pre>';
		#print_r($teste);
		#echo 1; exit();
	}
	...
	/*private function init() {
		...
		$this->_schema		= 'FSW_OSS_PADRAO';
		$this->_name		= 'DOCUMENTO';
		$this->_schema_name	= $this->_schema.'.'.$this->_name;
		//$this->_db->query("ALTER SESSION SET NLS_DATE_FORMAT = 'DD/MM/YYYY HH24:MI:SS'");
	...
	public function getClassificacao($id){
		...
		$select = $this->_db->select()
						//->distinct()
	                   ->from($this->_name, array("ID","NOME","DESCRICAO","ATIVO"),$this->_schema)
	                   ->where("ID = ".$id);
